...
%%% std 
singular_th=0.012*10^5;  %%% purposely choose 8 PCs, set singular value threshold (PC component signidicance); 
good_PCs_ind=find(max(S)>singular_th); 
features_goodPCed=features_PCed(:,good_PCs_ind); %%% n*Num_goodPCsFeatures  

%%%% std data after PCA
feature_mean=mean(features_goodPCed);
feature_std=std(features_goodPCed);
features_goodPCed=(features_goodPCed-feature_mean)./(feature_std);

%good_feature_num=sig_features_ind(good_PCs_ind); %%% final selected feature
selected_feature_mean=feature_mean; %%% mean of the seleced features (by correlation matrix)  
selected_feature_std=feature_std;
selected_train_data=features_goodPCed; %%% 


%%% save data 
train_processed=struct; 
train_processed.description="features f iltered by correlation + PCA firstm then standardized(substracted mean), Y unfiltered";
...
save('data/train_data_processed_PCAall_beofre_std','train_processed'); 
